...
If `sandbox.provider` is `e2b`, `agent-bundle build` generates a temporary E2B build context (`/skills`, `/tools`, `e2b.Dockerfile`), runs `e2b template build`, and bakes the resolved template ref into `bundle.json`.
